...
sc <- scale_color_manual(values=c("#999999", "#E69F00", "#56B4E9", "#009E73", "#CC79A7"), 
                         name='Subsample: ')
setup <- c('allpredictors','meteoCatchAttr')#, 'qMeteoStatevars')

remove <- c('datetime', 'obs')
predNames <- read.csv('/Users/thijskalshoven/Documents/UU_data_science/6_THESIS_Streamflow_predictions/Data/GitHub/Magni-PCR-GLOBWB-RF-NEW/source/data/predictors/pcr_allpredictors_AU_low/pcr_allpredictors_5101201.csv') %>% 
...
for(i in 1:2){
  ...
  for(subsample in 1:5){
    ...
    trainDir <- paste0('/Users/thijskalshoven/Documents/UU_data_science/6_THESIS_Streamflow_predictions/Data/GitHub/OUTPUT_MAGNI/RF/2_train/subsample_',subsample,'/')
...
ggsave(paste0(outputDir, 'varImportance2.png'), trainingPlotSize, height=35, width=25, units='in', dpi=600)
