...
#     
#       tuning_dir <- paste0('/Users/thijskalshoven/Documents/UU_data_science/6_THESIS_Streamflow_predictions/Data/GitHub/OUTPUT_MAGNI/RF/1_tune/subsample_',subsample,'/')
#       
...
for(i in 1:2){
  for(subsample in 1:5){
    ...
    if(i==1){    
      ntreePlot <- ggplot(ntreePlotData, aes(x=mtry,y=value, group=subsample, color=subsample))+
        geom_line(size=0.5)+
        geom_point(aes(shape=subsample), size=1.5, alpha = 0.6, show.legend = F) +
        sc+
        ylim(0.0001, 0.00035)+
...
# Patch it together using only ntreePlotList
tuningPlot <- (ntreePlotList[[1]] + ntreePlotList[[2]]) +
...
ggsave(paste0(outputDir, 'tuningPlot.png'), tuningPlot, height = 3, width = 10, units = 'in', dpi = 600)

# For when thee setups are used 
# #patch it together
