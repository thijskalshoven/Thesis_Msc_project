...
for(subsample in 1:5){
  ...
  rf.eval.uncalibrated <- read.csv(paste0('/Users/thijskalshoven/Documents/UU_data_science/6_THESIS_Streamflow_predictions/Data/GitHub/OUTPUT_MAGNI/RF/3_validate/subsample_', subsample,
  ...
  #read meteocatchattr
  rf.eval.meteoCatchAttr <- read.csv(paste0('/Users/thijskalshoven/Documents/UU_data_science/6_THESIS_Streamflow_predictions/Data/GitHub/OUTPUT_MAGNI/RF/3_validate/subsample_', subsample,
  ...
  # rf.eval.qMeteoStatevars <- read.csv(paste0('/Users/thijskalshoven/Documents/UU_data_science/6_THESIS_Streamflow_predictions/Data/GitHub/OUTPUT_MAGNI/RF/3_validate/subsample_', subsample,
  ...
  #read allpredictors
  rf.eval.allpredictors <- read.csv(paste0('/Users/thijskalshoven/Documents/UU_data_science/6_THESIS_Streamflow_predictions/Data/GitHub/OUTPUT_MAGNI/RF/3_validate/subsample_', subsample,
                                           '/KGE_allpredictors.csv')) %>% 
    select(.,grdc_no, KGE_corrected, KGE_r_corrected, KGE_alpha_corrected, KGE_beta_corrected) %>% 
    rename(., KGE=KGE_corrected, KGE_r=KGE_r_corrected, KGE_alpha=KGE_alpha_corrected, KGE_beta=KGE_beta_corrected) %>% 
  ...
  subsample_KGE <- rbind(rf.eval.uncalibrated, rf.eval.allpredictors, rf.eval.meteoCatchAttr) #, rf.eval.qMeteoStatevars)
...
KGE_boxplot <- ggplot(plotData , mapping = aes(setup, value, fill=setup))+
  ...
  facetted_pos_scales(y = list(
    KGE_component == "KGE" ~ scale_y_continuous(position='right', limits = c(-35, 1)),
    KGE_component == "KGE_r" ~ scale_y_continuous(position='right', limits = c(-0.5, 1)),
    KGE_component == "KGE_alpha" ~ scale_y_continuous(position='right', limits = c(-0.5,20)),
    KGE_component == "KGE_beta" ~ scale_y_continuous(position='right', limits = c(-0.5,30))))
KGE_boxplot
